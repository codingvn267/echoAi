...
const SYSTEM_PROMPT = {
  image: `You turn images into text. If it is a photo of a document, transcribe it. 
          If it is not a document, describe it.`,
  pdf: "You transform PDF files into text.",
  html: "You transform content into markdown.",
};

export type ExtractTypeContentArgs = {
...
export async function extractTextContent(
  ...
  args: ExtractTypeContentArgs
): Promise<string> {
  const { storageId, filename, bytes, mimeType } = args;
  const url = await ctx.storage.getUrl(storageId);
  assert(url, "Failed to get storage URL");

  if (SUPPORTED_IMAGES_TYPES.some((type) => type === mimeType)) {
